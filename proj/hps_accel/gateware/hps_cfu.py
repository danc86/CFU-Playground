...
class OperandsBuffer(ConcatenatingBuffer):
    ...
    def __init__(self):
        super().__init__([(f'input_{i}', 32) for i in range(4)] +
                         [(f'filter_{i}', 32) for i in range(4)])
        # Rearrange the input streams generated by the superclass to match our
        # desired interface.
        self.input_data = [self.inputs[f'input_{i}'] for i in range(4)]
        self.filter_data = [self.inputs[f'filter_{i}'] for i in range(4)]
